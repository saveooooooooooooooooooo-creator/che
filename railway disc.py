...
BOT_DELETE_TIME = 5         # seconds for auto-delete
MAX_WARNINGS = 5
MUTE_DURATION = 300         # seconds
SPAM_MESSAGE_LIMIT = 5
SPAM_TIME_WINDOW = 7
MOD_LOG_CHANNEL = "mod-logs"  # channel name for permanent logs

# Slur detection patterns
SLUR_PATTERNS = [
...
async def warn_user(member, channel, reason):
    ...
    msg = await channel.send(f"⚠️ {member.mention} {reason}\nWarning {warnings}/{MAX_WARNINGS}")
...
async def warnings(interaction: discord.Interaction, user: discord.Member):
    ...
    await interaction.response.send_message(f"⚠️ {user.mention} has {count}/{MAX_WARNINGS} warnings.", delete_after=BOT_DELETE_TIME)
...
async def clearwarnings(interaction: discord.Interaction, user: discord.Member):
    if not admin_only(interaction):
        await interaction.response.send_message("❌ Admins only.", delete_after=BOT_DELETE_TIME)
        return
    user_warnings[user.id] = 0
    await interaction.response.send_message(f"✅ Warnings reset for {user.mention}.", delete_after=BOT_DELETE_TIME)
...
async def mute(interaction: discord.Interaction, user: discord.Member):
    ...
    await interaction.response.send_message(f"🔇 {user.mention} has been muted by admin.", delete_after=BOT_DELETE_TIME)
...
async def unmute(interaction: discord.Interaction, user: discord.Member):
    ...
    await interaction.response.send_message(f"🔊 {user.mention} has been unmuted by admin.", delete_after=BOT_DELETE_TIME)
